...
- **Сравнение армий** (`/army/`). Пользователь собирает до пяти армий по семь отрядов, а экран сводит каждую
...
- **Армий от 1 до 5** (`ARMY_RULES.minArmies`, `maxArmies`). Номер армии — её место в списке: убрали
  армию из середины — номера следующих сдвигаются. Ключ для React — `id`, а не номер.
...
└── Activity → ArmyScreen                 сравнение армий
    ...
    ├── ArmyPanel ×1–5                    армия: шапка, семь ячеек, индексы
...
- **Ранг выше VIII** — допиши массив `ROMAN` в [roman.ts](src/components/roman.ts). Им же нумеруются
